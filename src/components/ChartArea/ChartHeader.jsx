  ...
  return (
    <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
      <button
        ...
        className="lg:hidden touch-target p-2 rounded-xl bg-white shadow-sm border border-slate-200"
      >
      ...
      <div className="flex bg-white p-1.5 rounded-[2rem] shadow-sm border border-slate-200 overflow-x-auto no-scrollbar max-w-full">
        {chartTypes.map((item) => (
          <button
            ...
            className={`px-4 lg:px-6 py-2 lg:py-3 rounded-2xl flex items-center gap-2 transition-all whitespace-nowrap touch-target ${chartType === item.id ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-slate-400 hover:text-indigo-600'}`}
          >
